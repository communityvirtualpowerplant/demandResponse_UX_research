...
logging.basicConfig(filename='/home/drux/demandResponse_UX_research/logs/controls.log',format='%(asctime)s - %(levelname)-8s [%(filename)s:%(lineno)d] %(message)s',datefmt='%Y-%m-%d %H:%M:%S',level=logging.INFO)
...
async def main():
    ...
    try:
        ...
        stateDict={"csrp":{"baselineW":[0,0,0,0],"baselineTS":False,"now":False,"upcoming":False,"goalAvg":0,"monthlyVal":0,"count":0},
                    "dlrp":{"baselineW":[0,0,0,0],"baselineTS":False,"now":False,"upcoming":False,"goalAvg":0,"monthlyVal":0,"count":0},
